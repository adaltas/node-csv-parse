...
import {CastingContext, Info, Options, Parser, CsvError, CsvErrorCode} from '../lib/index'
...
describe('API Types', () => {
  ...
  describe('CsvError', () => {
    describe('Typescript definition is accurate', () => {
      let errorCode = CsvErrorCode.InconsistentRecordLength;

      it('Minimum', () => {
        const error = new CsvError(errorCode, "MESSAGE");
        
        error.code.should.eql(errorCode)
        error.message.should.eql("MESSAGE")
      })

      it('Multiple messages', () => {
        const error = new CsvError(errorCode, ["MESSAGE1", "MESSAGE2"])
      ...
      it('Supports contexts', () => {
        const error = new CsvError(errorCode, "MESSAGE", { testContext: { testProp: "testValue" } })
